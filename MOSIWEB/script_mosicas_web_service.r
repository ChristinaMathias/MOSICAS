# R script to run Mosiweb from R - 01/08/2020 by Jean-Christophe Soulié (CIRAD, UR78)
# alternative use of Mosiweb on https://smartis.re/MOSIWEB
...
call_web_service <- function(long, lat) {
...
points_pour_mosicas_brut <- read.csv("~/Admin/Sujet Stage/Florent/mosicas/pt_savanna_gps.csv", sep=";", dec = ".", ) #au format num_labo, longitude, latitude
...
for (lig in 1:nb_ligne) {
	...
	req <- call_web_service(long, lat)
	data_frame <- ws_transform(req, TRUE)
	output_filename <- paste0("~/Admin/Sujet Stage/Florent/mosicas/output/mosiweb_out_", point_labo[lig])
	output_filename <- paste0(output_filename, ".csv")
	write.csv(data_frame, output_filename)
}